...
if __name__ == '__main__':
    ...
    while True:
        ...
        showOriginal("Aperte B para setar o background", cv2.flip(frame, 1))
        ...
        if k == 27:  
            ...
            camera.release()
            print('Background capturado com sucesso')
            break
    ...
    while True:
        ...
        img = img[0:int(cap_region_y_end * frame2.shape[0]),
                int(cap_region_x_begin * frame2.shape[1]):frame2.shape[1]]  
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        blur = cv2.GaussianBlur(gray, (blurValue, blurValue), 0)
        ret, thresh = cv2.threshold(blur, threshold, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        
        showPrediction(thresh, prediction, score, action) 
